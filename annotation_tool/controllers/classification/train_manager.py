...
class TrainManager(QObject):
    ...
    def start_training(self):
        ...
        params = {
            "epochs": self.panel.spin_epochs.currentText(),
            "lr": self.panel.edit_lr.text(),
            "batch": self.panel.spin_batch.currentText(),
            # Extract only the raw device token from combo text, e.g. "cuda (GPU)" -> "cuda"
            "device": self.panel.combo_device.currentText().split(" ")[0],
            "workers": self.panel.spin_workers.currentText(),
        ...
        self.panel.btn_start_train.setEnabled(False)
        self.panel.btn_stop_train.setEnabled(True)
        self.panel.train_progress.setVisible(True)
        self.panel.train_progress.setValue(0)
        self.panel.lbl_train_status.setVisible(True)
        self.panel.lbl_train_status.setText("🚀 Starting Training Loop...")
        self.panel.train_console.clear()

        # Create and wire up the training worker thread
        self.worker = TrainWorker(self.config_path, params)
        self.worker.log_signal.connect(self._append_log)
        self.worker.progress_signal.connect(self.panel.train_progress.setValue)
        self.worker.status_msg_signal.connect(self.panel.lbl_train_status.setText)
    ...
    def _append_log(self, text):
        ...
        self.panel.train_console.append(text)
    ...
    def stop_training(self):
        ...
        if self.worker and self.worker.isRunning():
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                self.panel.btn_stop_train.setEnabled(False)
                self.panel.lbl_train_status.setText("🛑 Aborting...")
    ...
    def _on_train_finished(self, success, message):
        ...
        self.panel.btn_start_train.setEnabled(True)
        self.panel.btn_stop_train.setEnabled(False)
        self.panel.train_progress.setVisible(False)
        self.panel.lbl_train_status.setVisible(False)
