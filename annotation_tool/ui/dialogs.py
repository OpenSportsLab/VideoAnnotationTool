...
class RemoteModelRegistrationDialog(QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Register Remote Model")
        self.setModal(True)
        self.resize(560, 280)
        root = QVBoxLayout(self)
        ...
        self.task_combo.addItem("Localization", "localization")
        self.task_combo.addItem("Question / Answer", "question_answer")
        self.form.addRow("Task:", self.task_combo)

        self.source_combo = QComboBox(self)
        self.source_combo.addItem("Hugging Face", "huggingface")
        self.source_combo.addItem("Server-local", "local")
        self.form.addRow("Source:", self.source_combo)

        self.repository_edit = QLineEdit(self)
        self.repository_edit.setPlaceholderText("owner/model-name")
        self.form.addRow("Repository ID:", self.repository_edit)

        self.model_id_edit = QLineEdit(self)
        self.model_id_edit.setPlaceholderText("Optional; generated by the server")
        self.form.addRow("Model ID:", self.model_id_edit)
        self.weights_path_edit = QLineEdit(self)
        self.weights_path_edit.setPlaceholderText("Path visible to the server")
        self.form.addRow("Weights path:", self.weights_path_edit)
        self.config_path_edit = QLineEdit(self)
        self.config_path_edit.setPlaceholderText(
            "Optional for a model directory; otherwise server-visible path"
        )
        self.form.addRow("Config path:", self.config_path_edit)

        note = QLabel(
            "Registration starts immediately on the server. Server-local paths "
            "are not selected from this computer.",
            self,
        )
        note.setWordWrap(True)
        root.addWidget(note)
        ...
        buttons.rejected.connect(self.reject)
        root.addWidget(buttons)
        self.source_combo.currentIndexChanged.connect(self._update_source_rows)
        self._update_source_rows()

    def _update_source_rows(self, *_args):
        is_huggingface = self.source_combo.currentData() == "huggingface"
        self.form.setRowVisible(self.repository_edit, is_huggingface)
        for editor in (
            self.model_id_edit,
            self.weights_path_edit,
            self.config_path_edit,
        ):
            self.form.setRowVisible(editor, not is_huggingface)

    def payload(self) -> dict:
        task = str(self.task_combo.currentData() or "")
        if self.source_combo.currentData() == "huggingface":
            repository_id = self.repository_edit.text().strip()
            if (
                not repository_id
        ...
        if not weights_path:
            raise ValueError("Enter a server-visible weights path.")
        return {
    ...
    def _validate_and_accept(self):
        try:
            ...
            QMessageBox.warning(self, "Invalid Remote Model", str(exc))
            return
        self.accept()
...
class ApplicationSettingsDialog(QDialog):
    ...
    def __init__(
        ...
        self.inference_admin_token_edit = self.inference_setup_widget.admin_token_edit
    ...
    def set_remote_model_operation_busy(
        ...
        self.inference_setup_widget.set_remote_model_operation_busy(
            busy, message, success=success
        )

    def stop_remote_model_polling(self) -> None:
        self.inference_setup_widget.stop_remote_model_polling()
