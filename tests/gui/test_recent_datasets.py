...
def test_recent_projects_list_updates_after_successful_import(window, monkeypatch, synthetic_project_json):
    window.dataset_explorer_controller.remove_all_recent_project()
    ...
    recents = window.dataset_explorer_controller.get_recent_projects()
...
def test_recent_project_click_opens_dataset(window, monkeypatch, qtbot, synthetic_project_json):
    ...
    assert Path(window.dataset_explorer_controller.current_json_path).resolve() == project_json_path
...
def test_recent_projects_dedupe_order_and_limit(window, monkeypatch, tmp_path, synthetic_project_json):
    window.dataset_explorer_controller.remove_all_recent_project()
    max_recent_display = window.dataset_explorer_controller.get_max_recent_datasets_displayed()
    ...
    for idx in range(max_recent_display + 5):
        ...
        assert window.dataset_explorer_controller.open_project_from_path(str(project_path)) is True

    stored_history = window.dataset_explorer_controller.settings.value(window.dataset_explorer_controller.RECENT_DATASETS_KEY, [])
    ...
    assert window.dataset_explorer_controller.open_project_from_path(str(reopened_path)) is True
    recents_after_reopen = window.dataset_explorer_controller.get_recent_projects()
    ...
    stored_history_after_reopen = window.dataset_explorer_controller.settings.value(window.dataset_explorer_controller.RECENT_DATASETS_KEY, [])
...
def test_recent_projects_missing_file_open_removes_stale_entry(window, monkeypatch, tmp_path):
    ...
    window.dataset_explorer_controller._add_recent_project(str(missing_project))
    assert window.dataset_explorer_controller.get_recent_projects() == [str(missing_project)]
    ...
    loaded = window.dataset_explorer_controller.open_project_from_path(str(missing_project))
    assert loaded is False
    assert warning_calls["count"] == 1
    assert window.dataset_explorer_controller.get_recent_projects() == []
...
#     assert window.dataset_explorer_controller.open_project_from_path(str(classification_path)) is True
#     assert window.dataset_explorer_controller.open_project_from_path(str(localization_path)) is True
#     assert window.dataset_explorer_controller.open_project_from_path(str(description_path)) is True
#     assert window.dataset_explorer_controller.open_project_from_path(str(dense_path)) is True
...
#     assert window.dataset_explorer_controller.open_project_from_path(str(first_path)) is True
#     assert window.dataset_explorer_controller.open_project_from_path(str(second_path)) is True
#     window.show_welcome_view()
#     assert window.welcome_widget.recent_projects_list.count() == 2

#     settings_file = window.dataset_explorer_controller.settings.fileName()
